...
def load_qa_json(path: Path) -> list:
    """Load QA JSON and return list of QA dicts preserving order."""
    data = json.loads(path.read_text(encoding='utf-8'))
    qa_list = []
    for item in data:
        # ensure required fields exist
        if "pergunta" in item and ("resposta" in item or "resposta" in item):
            qa_list.append(item)
    return qa_list


def compare(agent_entries: list, qa_list: list):
    """Zip through agent entries and qa_list positionally (zip_longest).
    Each agent_entries item is (file_field, answer); each qa item is a dict.
    """
    from itertools import zip_longest
    rows = []
    for idx, (agent_item, qa_item) in enumerate(zip_longest(agent_entries, qa_list)):
        agent_file, agent_ans = ('', '') if agent_item is None else agent_item
        # normal case: have both agent and QA
        gold = qa_item.get('resposta', '') or ''
        expected_file = qa_item.get('arquivo') or qa_item.get('Arquivo') or ''

        document = agent_item[0]
        answer = agent_item[1]
        metrics = compute_metrics(answer, gold, use_bert=USE_BERT_FLAG)

        r = {
            'expected_file': expected_file,
            'agent_answer': answer,
            'gold': gold,
            'document_found': document == expected_file,
            **metrics
        }
        rows.append(r)
    return rows
...
def main():
    ...
    p.add_argument('--agent_csv', required=False, default='results_csv/Agent_Answers.csv')
    p.add_argument('--qa_json', required=False, default='datasets/qa_dataset.json')
    p.add_argument('--out', default='results_csv/comparison_report.csv')
    ...
    agent_by_file = parse_agent_csv(agent_path)
    qa_by_file = load_qa_json(qa_path)
    # expose flag to comparison scope
    global USE_BERT_FLAG
    USE_BERT_FLAG = bool(args.use_bert)
    rows = compare(agent_by_file, qa_by_file)
