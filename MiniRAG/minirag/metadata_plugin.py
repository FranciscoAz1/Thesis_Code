...
__all__ = ["minirag_generate_metadata", "MetadataExtractor"]
...
class MetadataExtractor:
    """Class to handle metadata extraction with lazy-loaded Presidio analyzer."""
    
    def __init__(self):
        self._analyzer = None
    
    def _get_presidio_analyzer(self):  # pragma: no cover - runtime optional
        """Instantiate a Presidio AnalyzerEngine lazily.
        ...
        try:
            ...
            if NlpEngineProvider is not None:
                try:
                    ...
                    logger.warning("Could not load spaCy model 'pt_core_news_sm'; falling back. Error: %s", e)
            if engine is not None:
                self._analyzer = AnalyzerEngine(nlp_engine=engine, supported_languages=["pt"])  # type: ignore
            else:
                # Fallback create analyzer (may still load EN internally) but restrict supported_languages when possible.
                try:
                    logger.info("Creating Presidio AnalyzerEngine fallback (no dedicated Portuguese spaCy model)")
                    ...
                    if NlpEngineProvider is not None:
                        try:
                            ...
                            engine = NlpEngineProvider(nlp_configuration=cfg_fallback).create_engine()
                            self._analyzer = AnalyzerEngine(nlp_engine=engine, supported_languages=["pt"])
                        except Exception:
                            self._analyzer = AnalyzerEngine(supported_languages=["pt"])
                    else:
                        self._analyzer = AnalyzerEngine(supported_languages=["pt"])
                except Exception:
                    logger.info("Creating fully default Presidio AnalyzerEngine (final fallback)")
                    self._analyzer = AnalyzerEngine()
            self._presidio_register_custom(self._analyzer)
            if self._analyzer:
                logger.debug("Presidio analyzer initialized with custom recognizers: %s", [r.name for r in self._analyzer.registry.recognizers][-5:])
                # Log which built-in recognizers are loaded
                builtin_recognizers = [r.name for r in self._analyzer.registry.recognizers if not r.name.endswith('Recognizer')]
                if builtin_recognizers:
                    logger.info("Built-in Presidio recognizers loaded: %s", builtin_recognizers[:10])  # Show first 10
        except Exception as e:
            logger.error("Failed to initialize Presidio analyzer: %s", e)
    ...
    def _presidio_register_custom(self, analyzer):  # pragma: no cover - optional
        """Register custom recognizers for domain-specific patterns (idempotent).
    ...
    def _extract_with_presidio(self, text: str) -> Dict[str, Any]:  # pragma: no cover - optional
        """Extract PII and business entities from text using Presidio analyzer.
    ...
    def generate_metadata(self, doc_id: str, text: str, path: Any) -> Dict[str, Any]:
        ...
        pres_ex = self._extract_with_presidio(text)  # rely solely on Presidio (no regex fallback)
...
# ---- public function ----
def minirag_generate_metadata(doc_id: str, text: str, path: Any, extractor: Optional[MetadataExtractor] = None) -> Dict[str, Any]:
    """Generate metadata for a document using various extraction techniques.
    ...
    Args:
        ...
        text: The text content of the document.
        path: The file path or path-like object of the document.
        extractor: Optional MetadataExtractor instance to use. If None, uses global analyzer.
    ...
    """
    if extractor:
        return extractor.generate_metadata(doc_id, text, path)
    else:
        raise ValueError("No valid extractor provided.")
        # Fallback to global analyzer for backward compatibility
        # Basic doc-level signals (still lightweight even when relying on Presidio for PII)
        # clean text for better extraction
        text = clean_text(text)
